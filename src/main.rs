...
impl Player {
    ...
    pub fn updete(&mut self, dt: f32) {
        let x_move: f32 = match (is_key_down(KeyCode::Left), is_key_down(KeyCode::Right)) {
            (true, false) => -1f32,
            (false, true) => 1f32,
            _ => 0f32,
        };
...
async fn main() {
    ...
    loop {
        match game_state {
            ...
            GameState::Game => {
                player.updete(get_frame_time());
                for ball in balls.iter_mut() {
                    ball.updete(get_frame_time());
                }
